...
class Bot(object):
    ...
    def get_bid(
        ...
        def competitors():
            ...
            for bot in bots:
                if bot['bot_name'] != 'u5591185':
                    ...
                    #remaining_budget_percentage = (sum(bot['paintings'].values()) / bot['budget']) * 100
                    if remaining_budget_ratio >= 125.125:
                        danger += 1
                    if sum(bot['paintings'].values()) >= 6:
                        close_to_finish += 1
                    if sum(bot['paintings'].values()) > 3:
                        competition_bots.append(bot['bot_name'])
                        competitor_info[bot['bot_name']] = bot['paintings']

            # adjust utility again?
            return danger, close_to_finish, highest_available_bid_ratio, competition_bots, competitor_info
        ...
        def update_utiltity(my_collection, look_ahead, competitors_info, average):
            ...
            # Calculate weights for painting based on frequency of artist in the next 10
            weights = {artist: max(0, (10  / max(1,frequency[artist])) / 10) for artist in set(frequency)}

            current_utilities = self.utility[current_round:current_round+look_ahead]
            ...
            for bot_name, bot_info in competitors_info.items():
                if bot_info[current_painting] < 1 or bot_info[current_painting] == 2:
                    of_interest = True
                if bot_info[current_painting] == 1:
                    safety_lock = 0
                    for artist in artists:
                        if bot_info[artist] > 1:
                            safety_lock += 1
                    if safety_lock >= 2:
                        of_interest = False
                    else:
            ...
            if competitors_competing > 0:
                #if weights[current_painting] >= 0.3:
                bid = min(130, 125 * (1 + (weights[current_painting] / 10)))
            else:
                bid = min(average, 125 * (1 - (weights[current_painting] / 5)))
        ...
        def decide_bid_value(average, competitors_info):
            # Decide the best bid value based on all the information we have each round!
        ...
        def create_bid(current_painting, my_collection, my_remaining_budget):
            ...
            danger, close_to_finish, highest_available_bid_ratio, competition_bots, competitors_info = competitors()
            #print("\n Current Competitor Situation. Danger: {}. Close to Finish: {}. HABR: {} \n ".format(danger, close_to_finish, highest_available_bid_ratio) )

            # NEED TO USE THE AVERAGE BIDS OF THE WINNING BOTS PER PAINTING TO DETERMINE BEST VALUE BID
            ...
            #bid = min(self.utility[current_round-1], average)
        ...
        bid = create_bid(current_painting, my_collection, current_budget)
